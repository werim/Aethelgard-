...
## Gate 5B-2 main startup contract regression harness note

Gate 5B-2 adds `tests/test_main_startup_contract.py` and `docs/gates/gate5b_main_startup_contract.md` as a test/docs-only startup contract guard. The harness runs `main.py` in a controlled subprocess and checks `foundation_runtime_initialized`, JSON `metadata`/`settings`, `PAPER_ONLY`, `RESEARCH_ONLY`, seed `42`, and absence of unsafe live, secret, exchange-order, market-fetch success, trade-signal, optimizer-result, PnL, win-rate, Sharpe, drawdown, execution-capability, or readiness claims.

A passing subprocess test may classify only bounded startup evidence as `MEASURED_PAPER_DRY_RUN`. Gate 5B-2 does not prove production readiness, live readiness, exchange safety, market-data correctness, data completeness, execution realism, strategy validity, or profitability. Gate 5B-2 remains PAPER_ONLY / RESEARCH_ONLY / NOT_LIVE_READY and does not change runtime behavior, strategy logic, optimizer behavior, execution-cost modeling, performance calculation, PAPER runtime behavior, exchange mutation, exchange behavior, or readiness status. No live trading, secret request/read/exposure, exchange connection, market fetch, order placement, exchange mutation, strategy alpha logic, performance calculation, production readiness approval, or live-readiness approval is added. Unknown execution costs are not zero. Missing evidence remains unavailable. Backtest performance alone does not prove production readiness.
