...
- records symbol, side, timestamp, price source, entry/stop/take-profit references, market input state, and execution-cost assumption evidence,
- preserves spread, fee, slippage, latency, and funding assumptions as explicit values or explicit unavailable reasons,
- marks missing or stale critical market context as fail-closed invalid,
- keeps missing execution-cost assumptions unavailable rather than zero,
- projects the same context snapshot into audit evidence for persistence round-trip checks.
